var imgW = 150;
var imgH = 100;

var imgWM = 1;
var imgHM = 1;

var imgWW;
var imgHH;

var customChoice = false;
var customImage = false;
var noRainbow = false;

var backgroundColor = "#040404";

var d = document;

var customImageFile;

d.body.style.background = backgroundColor;

var canvas = d.getElementById("myStage");

canvas.style.position = "absolute";
canvas.style.left = 0;
canvas.style.right = 0;
canvas.style.top = 0;
canvas.style.bottom = 0;
canvas.style.width = "100%";
canvas.style.height = "100%";
canvas.style.zIndex = 3;

var ctx = canvas.getContext( '2d' ),
	w = canvas.width = window.innerWidth,
	h = canvas.height = window.innerHeight;

var hue = 20;

let moveX, moveY;

var xDir = "left",
	yDir = "down";

let spood = 10

var baseImage = "data:image/png;base64,";

var base_image = new Image();
base_image.src = baseImage;

var imgX = Math.floor(Math.random() * (w - imgW));
var imgY = Math.floor(Math.random() * (h - imgH));

function getUserData() {
	window.wallpaperPropertyListener = {
		applyUserProperties : function (props) {
			if (props.speed) {
				spood = props.speed.value
				speedRegulator();
			}
			if (props.imagesize) {
				imgWM = (props.imagesize.value / 10);
				imgHM = (props.imagesize.value / 10);
				updateImage();
			}
			if (props.applycustomsize) {
				customChoice = props.applycustomsize.value;
				updateImage();
			}
			if (props.customimage) {
				customImageFile = 'file:///' + props.customimage.value;
			}
			if (props.applycustomimage) {
				customImage = props.applycustomimage.value;
				if (customImage) {
					base_image.src = customImageFile;
				} else {
					base_image.src = baseImage;
				}
			}
			if (props.disablerainbow) {
				noRainbow = props.disablerainbow.value;
			}
			if (props.backgroundcolor) {
				var color = props.backgroundcolor.value.split(' ');
				color = color.map(function(c) {
					return Math.ceil(c * 255);
				});
				backgroundColor = 'rgb(' + color + ')';
			}
		},
	};
}

moveY = 1 * spood;

window.requestAnimFrame =
	window.requestAnimationFrame       ||
	window.webkitRequestAnimationFrame ||
	window.mozRequestAnimationFrame    ||
	window.oRequestAnimationFrame      ||
	window.msRequestAnimationFrame     ||
	function( callback ){
		window.setTimeout(callback, 1000 / 30);
	};

function speedRegulator() {
	let spaad = spood / 10
	w = canvas.width = window.innerWidth,
	h = canvas.height = window.innerHeight;
	if ( w - imgW === h - imgH ) {
		moveX = 1.5 * spaad;
		moveY = 1 * spaad;
	} else {
		moveX = 1 * spaad;
		moveY = 1 * spaad;
	}
}

function updateImage() {
	if (customChoice) {
		imgWW = imgW * imgWM;
		imgHH = imgH * imgHM;
	} else {
		imgWW = imgW;
		imgHH = imgH;
	}
}

var init = function(){
	
	if (noRainbow) {
		ctx.globalCompositeOperation = "source-over";
		
		ctx.fillStyle = backgroundColor;
		ctx.fillRect(0, 0, w, h);
	} else {
		ctx.globalCompositeOperation = "source-over";

		ctx.fillStyle = backgroundColor;
		ctx.fillRect(0, 0, w, h);

		ctx.globalCompositeOperation = "source-over";

		ctx.fillStyle = "hsla(" + hue + ", 50%, 45%, 1)";
		ctx.fillRect(0, 0, w, h);

		ctx.globalCompositeOperation = "destination-in";
	}
	
	ctx.drawImage(base_image, imgX, imgY, imgWW, imgHH); // Draw Screen
	
	if (xDir == "left") {
		if (imgX >= w - imgWW) {
			xDir = "right";
			imgX = imgX - moveX;
		} else {
			imgX = imgX + moveX;
		}
	} else {
		if (imgX <= 0) {
			xDir = "left";
			imgX = imgX + moveX;
		} else {
			imgX = imgX - moveX;
		}
	}
	if (yDir == "down") {
		if (imgY >= h - imgHH) {
			yDir = "up";
			imgY = imgY - moveY;
		} else {
			imgY = imgY + moveY;
		}
	} else {
		if (imgY <= 0) {
			yDir = "down";
			imgY = imgY + moveY;
		} else {
			imgY = imgY - moveY;
		}
	}
	hue = ( hue + 0.5 ) % 360;

	requestAnimFrame(init);

}

getUserData();
speedRegulator();
init();

base_image.onload = function() {
	window.addEventListener("resize", speedRegulator);
	imgW = base_image.width / (base_image.height / 100);
	updateImage();
};